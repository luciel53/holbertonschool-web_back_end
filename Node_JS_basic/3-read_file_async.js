      ...
      for (let i = 1; i < byLine.length; i += 1) {
        ...
        if (field) {
          ...
          if (studentsByField[field][firstname] === undefined) {
            studentsByField[field][firstname] = 1; // Add firstname to this field list with count 1
          } else {
            studentsByField[field][firstname] += 1; // Increment count for firstname in field
          }
        }
        totalStudents += 1;
      }
